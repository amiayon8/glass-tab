...
const defaultShortcuts: FrequentlyVisited[] = [
  {
...
const wallpaperProviders = ["NASA", "Bing"];

interface GlassTabClientProps {
  initialWallpaperData?: WallpaperData;
  initialProvider?: string;
}

export default function GlassTabClient({
  initialWallpaperData,
  initialProvider = "NASA",
}: GlassTabClientProps) {
  const [wallpaperData, setWallpaperData] = useState<WallpaperData | undefined>(initialWallpaperData);
  const [wallpaperFetched, setWallpaperFetched] = useState<boolean>(Boolean(initialWallpaperData?.hdurl));
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [frequentlyVisited, setFrequentlyVisited] = useState<FrequentlyVisited[]>(defaultShortcuts);
  ...
  const [wallpaperProvider, setWallpaperProvider] = useState<string>(initialProvider);
  const [wallpaperDropdownOpen, setWallpaperDropdownOpen] = useState<boolean>(false);

  const [editingSite, setEditingSite] = useState<FrequentlyVisited | null>(null);
  ...
  useEffect(() => {
    ...
    if (storedWallpaperProvider) {
      ...
      if (provider && provider !== wallpaperProvider) {
  ...
  useEffect(() => {
    const fetchWallpaper = async () => {
      if (wallpaperProvider === initialProvider && initialWallpaperData?.hdurl) {
        setWallpaperData(initialWallpaperData);
    ...
    fetchWallpaper();
  }, [wallpaperProvider, initialProvider, initialWallpaperData]);
  ...
  return (
    <div className="relative w-dvw h-dvh">
      <div className="z-0 fixed inset-0 bg-black size-full transition-all">
        {wallpaperFetched && wallpaperData?.hdurl && (
          <Image
            src={wallpaperData.hdurl}
            alt={wallpaperData.title || "Background wallpaper"}
            fill
            priority
            sizes="100vw"
            quality={85}
      ...
      <div className="top-40 left-1/2 absolute flex flex-row flex-wrap justify-center items-center gap-x-2 gap-y-4 w-full max-w-[90vw] text-center -translate-x-1/2">
        {frequentlyVisited.map((site, index) => (
          <div
            ...
            className={`flex flex-col items-center w-16 cursor-grab active:cursor-grabbing transition-all duration-200 ${draggedIndex === index ? "opacity-25 scale-90" : ""
              }`}
          >
            <a
              href={site.url}
            ...
            <p className="mt-1 w-full text-[11px] text-white/90 text-center wrap-break-word leading-tight select-none">
              {site.title}
            </p>
          </div>
        ))}
        {frequentlyVisited.length <= 14 && (
          <div className="flex flex-col items-center w-16">
            ...
            <p className="mt-1 w-full text-[11px] text-white/90 text-center leading-tight select-none">
              Add
            </p>
          </div>
        )}
        ...
        <div className="top-28 right-5 z-50 absolute bg-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-2xl border border-white/15 rounded-3xl w-96 transition-all duration-300">
          ...
          <div className="relative space-y-5 p-6">
            <div className="flex flex-row justify-between items-center gap-4">
              <div>
                <h2 className="font-semibold text-white text-lg">Add Shortcut</h2>
      ...
      {editingSite && (
        <div className="top-28 right-5 z-50 absolute bg-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-2xl border border-white/15 rounded-3xl w-96 transition-all duration-300">
          ...
          <div className="relative space-y-5 p-6">
            <div className="flex flex-row justify-between items-center gap-4">
              <div>
                <h2 className="font-semibold text-white text-lg">Edit Shortcut</h2>
                <p className="mt-1 text-white/60 text-sm">Modify the website details.</p>
              </div>
              <button
                className="text-gray-200 hover:text-white hover:scale-101 transition-all duration-300"
            ...
            <div className="space-y-3">
              ...
              <input
                onChange={(e) =>
                  setEditingSite((prev) =>
                    prev ? { ...prev, iconUrl: e.target.value } : null
                  )
                }
      ...
      {customizing && (
        <div className="top-18 right-5 z-50 absolute bg-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-2xl border border-white/15 rounded-3xl w-96 transition-all duration-300">
          ...
          <div className="relative space-y-5 p-6">
            <div className="flex flex-row justify-between items-center gap-4">
              <div>
                <h2 className="font-semibold text-white text-lg">Customize GlassTab</h2>
              </div>
              <button
                className="text-gray-200 hover:text-white hover:scale-101 transition-all duration-300"
              ...
              <div className="relative space-y-2">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <FaChevronDown
                      className={`size-3.5 transition-transform duration-300 ${wallpaperDropdownOpen ? "rotate-180" : ""
                        }`}
                    />
                  </button>
      ...
      {wallpaperData?.link && (
        ...
        >
          ...
          <span className="text-[10px] text-gray-200">{wallpaperData.copyright}</span>
        </a>
      )}
    </div>
  );
